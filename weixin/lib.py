...
class BaseHttpClient(object):
    ...
    def postXmlSSLCACurl(self, xml, url, second=30, cert=True, post=True,cacert=True):
        return self.postXmlCASSL(xml, url, second) 
...
class CurlClient(BaseHttpClient):
    ...
    def postXmlCASSL(self, xml, url, second=30, cert=True, post=True,cacert=True):
        """使用证书"""
        ...
        if cacert:
            print '加载CA证书',WxPayConf_pub.ROOT_CACERT_PATH
            self.curl.setopt(pycurl.SSL_VERIFYPEER, 1) # =看网上这么设置
            self.curl.setopt(pycurl.SSL_VERIFYHOST, 2)
            self.curl.setopt(pycurl.SSLCERTTYPE, "P12") 
            self.curl.setopt(pycurl.SSLCERT,WxPayConf_pub.ROOT_CACERT_PATH)
            self.curl.setopt(pycurl.SSLCERTPASSWD,WxPayConf_pub.CACERT_PASSWD)
        #post提交方式
        if post:
    ...
    def postXmlSSL(self, xml, url, second=30, cert=True, post=True):
        ...
        if cert:
            ...
            self.curl.setopt(pycurl.SSLCERT, WxPayConf_pub.SSLCERT_PATH)
        # if cacert:
        #     self.curl.setopt(pycurl.CAINFO,WxPayConf_pub.ROOT_CACERT_PATH)
        #post提交方式
        if post:
            self.curl.setopt(pycurl.POST, True)
            self.curl.setopt(pycurl.POSTFIELDS, xml)
